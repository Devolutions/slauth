...
use crate::u2f::{
    ...
    proto::{
        constants::*,
        raw_message::{
            apdu, AuthenticateRequest, AuthenticateResponse, Message, RegisterRequest, RegisterResponse, VersionRequest, VersionResponse,
        },
    },
};
...
pub fn register(req: RegisterRequest, attestation_cert: &[u8], attestation_key: &[u8]) -> Result<(RegisterResponse, SigningKey), Error> {
    let RegisterRequest { challenge, application } = req;
...
pub fn sign(req: AuthenticateRequest, signing_key: &SigningKey, counter: u32, user_presence: bool) -> Result<AuthenticateResponse, Error> {
    ...
    match control {
        ...
        _ => Err(Error::U2FErrorCode(U2F_SW_INS_NOT_SUPPORTED)),
    }
}
...
impl U2FSToken {
    pub fn handle_apdu_request_with_timeout(&self, req: apdu::Request, timeout: Option<Duration>) -> apdu::Response {
        let res = match req.command_mode {
            U2F_REGISTER => RegisterRequest::from_apdu(req).and_then(|reg| self.register(reg, timeout).and_then(|rsp| rsp.into_apdu())),
            U2F_AUTHENTICATE => {
    ...
    fn register(&self, req: RegisterRequest, timeout: Option<Duration>) -> Result<RegisterResponse, Error> {
        if self
            .presence_validator
            .check_user_presence(timeout.unwrap_or_else(|| Duration::from_secs(10)))
        {
