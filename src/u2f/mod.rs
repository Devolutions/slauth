...
fn test() {
    ...
    let (rsp, signing_key) = web_req.register(origin.to_string(), base64::decode(ATT_CERT).unwrap().as_slice(), base64::decode(ATT_PKEY).unwrap().as_slice()).expect("Unable to register");

    let registration_rsp = if let Response::Register(reg) = rsp {
        reg
    } else {
        assert!(false);
        return;
    ...
    let server_sign_request = U2fRequestBuilder::sign().app_id(APP_ID.to_string()).challenge("987654321".to_string()).timeout_sec(82).registered_keys(vec![registration.get_registered_key()]).build().expect("Unable to build U2fRequest Sign");
    ...
    let rsp = web_sign_req.sign(&signing_key, origin.to_string(), 1, true).expect("Unable to sign");

    let sign_rsp = if let Response::Sign(sig) = rsp {
        sig
    } else {
        assert!(false);
        return;
    ...
    assert!(sign_rsp.validate_signature(registration.pub_key.as_slice()).expect("Unable to validate signature"));
}